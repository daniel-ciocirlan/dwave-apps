...
class MyEvaluatorObject ( object ):
    ...
    # initialize the return values as a list
        ...
        for i in range(numStates):
            ...
            result = 0
            for index in range(stateLen):
                result = result + state_bin[index] * self.superset[index]

            result = (result - self.target) ** 2

            superprod = 1
            for bit in state_bin:
                superprod = superprod * (1 - bit)
